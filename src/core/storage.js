/**
 * CEP Online Mock Test Platform - Core Storage Engine
 * Persistent local storage driver with dual-mode support for Firestore adapter.
 */

import { SEED_QUESTIONS } from './seedQuestionsData.js';

const STORAGE_KEYS = {
    STUDENTS: 'cep_platform_students',
    QUESTIONS: 'cep_platform_questions',
    EXAMS: 'cep_platform_exams',
    TEST_SESSIONS: 'cep_platform_test_sessions',
    SUBMISSIONS: 'cep_platform_submissions',
    CURRENT_USER: 'cep_platform_current_user',
    SYSTEM_SETTINGS: 'cep_platform_settings'
};
...
const SEED_EXAMS = [
    ...
    },
    {
        ...
        description: 'TCS iON CBT Exam pattern for Forest Guard recruitment.',
        totalMarks: 120,
        totalQuestions: 60,
        durationMinutes: 90,
...
export class StorageEngine {
    ...
    registerStudent(name, email, password, enrollmentId = '') {
        const students = this.getStudents();
        ...
        const newStudent = {
            ...
            email,
            password,
            enrollmentId: enrollmentId || 'SIGMA-' + Math.floor(1000 + Math.random() * 9000),
            role: 'student',
            status: 'active',
            allowedTests: 10,
            completedTests: 0,
    ...
    saveStudent(studentData) {
        ...
        if (studentData.id) {
            ...
            updated = {
                ...
                enrollmentId: studentData.enrollmentId || 'SIGMA-' + Math.floor(1000 + Math.random() * 9000),
                role: 'student',
    ...
    // --- QUESTION BANK CRUD & BULK UPLOAD ---
    getQuestions() {
